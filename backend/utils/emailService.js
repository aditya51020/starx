...
export const sendInquiryEmail = async (inquiry) => {
    ...
    try {
        const mailOptions = {
            ...
            subject: `New Inquiry from ${escapeHTML(inquiry.name)} - StarX Properties`,
            html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <div style="background-color: #f3f4f6; padding: 20px; border-radius: 8px; margin: 20px 0;">
            ...
            ${inquiry.propertyInterest ? `<p><strong>Interest:</strong> ${escapeHTML(inquiry.propertyInterest)}</p>` : ''}
            ${inquiry.location ? `<p><strong>Location:</strong> ${escapeHTML(inquiry.location)}</p>` : ''}
            ${inquiry.propertyId ? `<p><strong>Property ID:</strong> ${escapeHTML(inquiry.propertyId)}</p>` : ''}
            ${inquiry.propertyTitle ? `<p><strong>Property Title:</strong> ${escapeHTML(inquiry.propertyTitle)}</p>` : ''}
          </div>
